...
void ProfilePageExt::onProfileUpdateHttpResponse(CCHttpClient* client, CCHttpResponse* response) {
    ...
    if (S_OK == URLDownloadToFile(NULL, responseString.c_str(), "gtps\\Resources\\UserContent\\.ProfileImage", 0, NULL))
    {
...
void __fastcall ProfilePage_loadPageFromUserInfo_H(ProfilePageExt* self, void*, GJUserScore* a2) {
    ...
    ProfileUpdateHttp->setUrl(("http://user95401.undo.it/gtps/core-modules/ProfileImage/get.php?accountID=" + std::string(ProfileUpdateHttp->getTag())).c_str());
